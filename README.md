...
This service uses LangChain with the Looker SQL Agent and OpenAI to provide answers to specific data-related questions.
...
2. Build and push the container image:
    ...
    gcloud builds submit --tag \
      YOUR_REGION-docker.pkg.dev/YOUR_PROJECT_ID/YOUR_REPOSITORY/ra-databot
    ```
...
4. Test the service by using cURL to send a question:

    ```sh
    curl -X POST -H "Content-Type: application/json" \
      --data '{"question":"and what was it in April 2024?"}' \
      https://YOUR_REGION-YOUR_PROJECT_ID.run.app
    ```

5. Within the chatbot-plugin directory, edit the chatbot.js file and add your Cloud Run service endpoint:

    ```sh
     fetch('https://YOUR_REGION-YOUR_PROJECT_ID.run.app', requestOptions)
