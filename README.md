...
This project deploys a Cloud Run service that uses LangChain with the Looker SQL Agent and OpenAI to provide answers to specific data-related questions.
...
- Functions in the `config.py` help retrieve values from Secret Manager for the Cloud Run service.
...
2. Download the Looker Avatica JDBC driver from Looker and place the JAR file at `drivers/avatica.jar`.

3. Build and deploy the Cloud Run service:

    ```sh
    gcloud builds submit --tag gcr.io/$GCP_PROJECT/ra-databot
    gcloud run deploy ra-databot \
      --image gcr.io/$GCP_PROJECT/ra-databot \
      --platform=managed \
      --region YOUR_REGION \
...
4. Test the Cloud Run service by using cURL to send a question:

    ```sh
    curl -X POST -H "Content-Type: application/json" \
    --data '{"question":"and what was it in April 2024?"}' \
    https://YOUR_REGION-run-$GCP_PROJECT.a.run.app
    ```

5. Within the `chatbot-plugin` directory, edit the `chatbot.js` file and add your Cloud Run service endpoint:

    ```sh
     fetch('https://YOUR_REGION-run-$GCP_PROJECT.a.run.app', requestOptions)
...
3. Back-end service is a serverless Cloud Run container which then takes the question and passes it to a LangChain SQL Agent that in-turn passes the question to OpenAI’s GPT4-Turbo LLM, prefixed with the prompt below (change this to be appropriate for your data) to come up with a strategy to answer the user’s question:
...
There are other methods to secure access to a Cloud Run REST API but they are really outside the scope of this code example (and something you'd really want to discuss and have implemented by your dev team).
